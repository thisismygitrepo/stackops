...
rm -rfd .venv
uv add --no-cache 'cryptography' 'fire' 'gitpython' 'joblib' 'psutil' 'pyyaml' 'questionary' 'randomname' 'rclone-python' 'requests' 'rich' 'tenacity' 'typer'
uv add --no-cache --group plot 'ipykernel' 'ipython' 'jupyterlab' 'jupyter-ai' 'kaleido' 'matplotlib' 'nbformat' 'numpy' 'plotly' 'polars' 'python-magic' 'sqlalchemy'
...
uv add --no-cache --group plot 'duckdb' 'ipykernel' 'ipython' 'jupyterlab' 'jupyter-ai' 'kaleido' 'matplotlib' 'nbformat' 'numpy' 'plotly' 'polars' 'python-magic' 'sqlalchemy'
