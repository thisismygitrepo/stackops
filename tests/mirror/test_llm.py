...
import llm
from support.models import RuntimeContext
...
def test_cli_installs_and_explicitly_refreshes_install_only_profile(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(llm, "_runtime_context", lambda: context)
    ...
    install_result = runner.invoke(llm.app, ["auggie", "--profile", "work"])
    ...
    refresh_result = runner.invoke(llm.app, ["auggie", "--refresh", "--profile", "work"])
...
def test_cli_auto_refreshes_codex_by_composite_user_and_workspace_identity(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = CliRunner().invoke(llm.app, ["x", "--refresh"])
...
def test_cli_reports_managed_login_agent_without_accessing_profile_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(llm, "_runtime_context", lambda: _runtime_context(home=tmp_path))

    result = CliRunner().invoke(llm.app, ["antigravity"])
...
def test_cli_rejects_unknown_agent() -> None:
    result = CliRunner().invoke(llm.app, ["unknown-agent"])
