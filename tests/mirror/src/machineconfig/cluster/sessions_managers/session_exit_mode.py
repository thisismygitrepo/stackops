...
from machineconfig.cluster.sessions_managers import session_exit_mode
...
def test_build_powershell_exit_mode_command_parts_back_to_shell_uses_no_exit() -> None:
    ...
    assert "$machineconfigCommand = 'Write-Host ''hi'''" in decoded_script
...
def test_build_powershell_exit_mode_command_parts_kill_window_exits() -> None:
    ...
    assert decoded_script.endswith("exit $machineconfigExitCode")
