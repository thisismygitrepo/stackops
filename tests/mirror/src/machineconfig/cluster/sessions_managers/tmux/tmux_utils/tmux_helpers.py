...
def test_normalize_cwd_and_attach_command(
...
def test_validate_layout_config_rejects_blank_tab_name() -> None:
    bad_layout: LayoutConfig = {
        "layoutName": "bad",
        "layoutTabs": [{"tabName": " ", "startDir": "/work", "command": "echo hi"}],
    }
...
def test_list_tmux_window_names_handles_missing_and_other_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_missing(
        args: list[str],
...
def test_build_tmux_commands_wraps_non_shell_exit_modes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        fake_build_tmux_exit_mode_command,
    )
...
def test_build_tmux_merge_commands_overwrite_replaces_matching_windows(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_list_tmux_window_names(session_name: str) -> set[str]:
        _ = session_name
        return {"editor"}

    def fake_generate_random_suffix(length: int) -> str:
        _ = length
        return "abc12345"
    ...
    assert any(
        "editor__machineconfig_replace__abc12345" in command for command in commands
    )
    assert "tmux kill-window -t alpha:editor" in commands
    assert (
        "tmux rename-window -t alpha:editor__machineconfig_replace__abc12345 editor"
        in commands
    )


def test_check_tmux_session_status_handles_no_server_and_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_missing(
        args: list[str],
