...
import machineconfig.cluster.remote.remote_machine as remote_machine_module
...
def test_init_rejects_duplicate_workload_params_sources(tmp_path: Path) -> None:
    ...
    config = RemoteMachineConfig(job_id="job-1", base_dir=str(tmp_path), workload_params=workload)
...
def test_check_job_status_returns_results_path_for_completed_local_job(tmp_path: Path) -> None:
    config = RemoteMachineConfig(job_id="job-3", base_dir=str(tmp_path))
