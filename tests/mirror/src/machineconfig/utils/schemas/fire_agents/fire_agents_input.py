...
from machineconfig.utils.schemas.fire_agents import fire_agents_input as fire_agents_input_module


def test_search_strategies_literal_options_match_supported_modes() -> None:
    assert get_args(fire_agents_input_module.SEARCH_STRATEGIES) == (
        "file_path",
        "keyword_search",
        "filename_pattern",
    )
...
def test_file_path_search_input_requires_both_runtime_keys() -> None:
    assert fire_agents_input_module.FilePathSearchInput.__required_keys__ == frozenset(
        {"file_path", "separator"}
    )
