...
from machineconfig.utils.schemas.layouts import layout_types as layout_types_module


def test_serialize_layouts_to_file_creates_schema_annotated_json(
    tmp_path: Path,
) -> None:
    ...
    layout_types_module.serialize_layouts_to_file(
        layouts=layouts,
        version="0.1",
...
def test_serialize_layouts_to_file_replaces_matching_layout_and_appends_new_one(
    tmp_path: Path,
) -> None:
    layout_path = tmp_path / "configs" / "layout.json"
...
def test_substitute_home_expands_tab_paths_and_shorthand_commands() -> None:
    ...
    assert substituted == [
        {
            "tabName": "fire",
            "startDir": f"{home}/work",
            "command": "~/.config/machineconfig/scripts/wrap_mcfg fire launch",
        },
        {
            "tabName": "seek",
            "startDir": f"{home}/research",
            "command": "~/.config/machineconfig/scripts/wrap_mcfg seek query",
            "tabWeight": 2,
        },
        {
            "tabName": "term",
            "startDir": "/tmp",
            "command": "~/.config/machineconfig/scripts/wrap_mcfg terminal shell",
        },
    ]
