...
def test_substitute_home_expands_tab_paths_and_shorthand_commands() -> None:
    ...
    assert substituted == [
        {"tabName": "fire", "startDir": f"{home}/work", "command": "~/.config/machineconfig/scripts/wrap_mcfg fire launch"},
        {"tabName": "seek", "startDir": f"{home}/research", "command": "~/.config/machineconfig/scripts/wrap_mcfg seek query", "tabWeight": 2},
        {"tabName": "term", "startDir": "/tmp", "command": "~/.config/machineconfig/scripts/wrap_mcfg terminal shell"},
    ]
