...
from machineconfig.utils.installer_utils import installer_offline as offline
...
def test_uv_helper_functions_parse_python_and_filter_links(tmp_path: Path) -> None:
    ...
    (tool_root / "bin" / "machineconfig").write_text(
        "#!/usr/bin/env sh\n",
        encoding="utf-8",
    )
    (tool_root / "bin" / "python").symlink_to(python_root / "bin" / "python3.14")
    (install_path / "machineconfig").symlink_to(tool_root / "bin" / "machineconfig")
    (install_path / "mcfg").symlink_to(install_path / "machineconfig")
...
def test_export_creates_linux_archive_with_uv_bundle(
    ...
    monkeypatch.setattr(offline, "UV_TOOL_BINARIES", ["machineconfig"])
    ...
    assert "uv_bundle/tools/machineconfig/pyvenv.cfg" in names
