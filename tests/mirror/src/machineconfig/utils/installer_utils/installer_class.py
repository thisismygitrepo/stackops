...
def _make_installer_data(
    *, app_name: str = "My Tool", repo_url: str = "https://github.com/owner/repo", linux_pattern: str | None = "tool-{version}-linux-amd64.tar.gz"
) -> InstallerData:
    return {
        "appName": app_name,
        "license": "MIT",
...
def test_install_robust_returns_skipped_when_tool_already_exists(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        installer, "_read_installed_version", lambda exe_name: (_ for _ in ()).throw(AssertionError("skip path should not read version"))
    )

    result = installer.install_robust(InstallRequest(version=None, update=False))

    assert result == {"kind": "skipped", "appName": "My Tool", "exeName": "mytool", "emoji": "⏭️", "detail": "already installed, skipped"}


def test_install_robust_returns_updated_when_version_changes(monkeypatch: pytest.MonkeyPatch) -> None:
    installer = installer_class.Installer(_make_installer_data())
    observed_requests: list[InstallRequest] = []
    installed_versions = ["1.0.0", "2.0.0"]

    def fake_resolve_install_request(install_request: InstallRequest) -> tuple[InstallTarget, InstallRequest]:
    ...
    assert result == {"kind": "updated", "appName": "My Tool", "exeName": "mytool", "emoji": "🤩", "oldVersion": "1.0.0", "newVersion": "2.0.0"}


def test_install_robust_returns_failed_on_install_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    installer = installer_class.Installer(_make_installer_data())
    ...
    monkeypatch.setattr(
        ...
        lambda install_request: (InstallTarget(installer_kind="github_release", installer_value="tool-{version}.tar.gz"), install_request),
    )
    monkeypatch.setattr(installer, "_read_installed_version", lambda exe_name: "1.0.0")

    def fake_install_requested_with_target(install_target: InstallTarget, install_request: InstallRequest) -> None:
    ...
    assert result == {"kind": "failed", "appName": "My Tool", "exeName": "mytool", "emoji": "❌", "error": "boom"}


def test_get_github_release_accepts_filename_variants(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda username, repository, version: {
            "tag_name": "v1.2.3",
            "assets": [
                {"name": "tool_v1.2.3_linux_amd64.tar.gz", "browser_download_url": "https://downloads.example/tool_v1.2.3_linux_amd64.tar.gz"}
            ],
        },
    )
