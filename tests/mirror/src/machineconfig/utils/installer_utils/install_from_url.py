...
import machineconfig.utils.installer_utils.install_from_url as install_from_url
...
def test_finalize_install_moves_binary_and_records_version(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def fake_find_move_delete_linux(*, downloaded: PathExtended, tool_name: str | None, delete: bool, rename_to: str | None) -> PathExtended:
    ...
    install_from_url._finalize_install(
        repo_name="owner/my-tool", asset_name="tool.tar.gz", version="v1.2.3", extracted_path=extracted_path, console=console
    )
...
def test_install_from_github_url_selects_asset_and_finalizes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    console = ConsoleSpy(messages=[])
    ...
    def fake_finalize(repo_name: str, asset_name: str | None, version: str, extracted_path: PathExtended, console: ConsoleSpy) -> None:
    ...
    monkeypatch.setattr("machineconfig.utils.options.choose_from_options", fake_choose_from_options)
...
def test_install_from_binary_url_delegates_to_finalize(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    finalized_calls: list[tuple[str, str | None, str, PathExtended]] = []
    ...
    monkeypatch.setattr(
        install_from_url,
        "_finalize_install",
        lambda repo_name, asset_name, version, extracted_path, console: finalized_calls.append((repo_name, asset_name, version, extracted_path)),
    )
    monkeypatch.setattr("rich.console.Console", lambda: console)
