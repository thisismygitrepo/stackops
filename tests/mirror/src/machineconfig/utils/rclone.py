...
from machineconfig.utils import rclone as rclone_module


def test_rclone_hint_distinguishes_config_errors_from_missing_paths() -> None:
    config_hint = rclone_module._rclone_hint(
        stdout="",
        stderr="directory not found because it didn't find section in config file",
    )
    assert config_hint == (
        "The configured rclone remote could not be resolved. Verify the remote name "
        "and your rclone config."
    )

    missing_hint = rclone_module._rclone_hint(
...
def test_is_missing_remote_path_error_uses_combined_output() -> None:
    error = rclone_module.RcloneCommandError(
...
def test_copy_sync_and_bisync_build_expected_commands(
    ...
    assert observed == [
        (
            ["rclone", "copyto", "local.txt", "remote:file.txt", "--transfers=4", "--progress"],
            True,
            True,
        ),
        (
            ["rclone", "sync", "src", "dst", "--transfers=2", "--verbose", "--delete-during"],
            False,
            False,
        ),
        (
...
def test_link_returns_first_non_empty_line(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_run(
        command: list[str],
...
def test_link_raises_when_rclone_returns_no_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_run(
        command: list[str],
