...
def test_read_mapper_filters_current_os_and_splits_public_private_paths(
    tmp_path: Path,
    ...
    def fake_load_dotfiles_mapper(_path: Path) -> dict[str, dict[str, dict[str, object]]]:
        return {
            "git": {
                ...
                "windows_only": {
                    "original": "C:/Users/alex/_gitconfig",
                    "self_managed": "CONFIG_ROOT/windows/gitconfig",
                    "os": ["windows"],
                },
            }
        }
...
def test_iter_operation_paths_returns_sorted_children_for_contents_mapper(
    tmp_path: Path,
) -> None:
    ...
    operation_paths = create_links_module._iter_operation_paths(
        config_file_default_path=default_dir,
        config_file_self_managed_path=managed_dir,
        contents=True,
        direction="up",
    )
    ...
    assert operation_paths == [
        (
            default_dir.joinpath("a-first.txt"),
            managed_dir.joinpath("a-first.txt"),
        ),
        (
            default_dir.joinpath("z-last.txt"),
            managed_dir.joinpath("z-last.txt"),
        ),
    ]


def test_iter_operation_paths_rejects_non_directory_contents_sources(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(NotADirectoryError, match="expects a directory"):
        create_links_module._iter_operation_paths(
            config_file_default_path=default_file,
            config_file_self_managed_path=managed_dir,
            contents=True,
            direction="up",
        )
...
def test_run_directional_operation_uses_expected_target_mapping(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    up_result = create_links_module._run_directional_operation(
    ...
    down_result = create_links_module._run_directional_operation(
        config_file_default_path=default_path,
        config_file_self_managed_path=managed_path,
    ...
    symlink_result = create_links_module._run_directional_operation(
        config_file_default_path=default_path,
        config_file_self_managed_path=managed_path,
    ...
    assert copy_calls == [
        (managed_path, default_path, "overwrite-default-path"),
        (default_path, managed_path, "backup-self-managed"),
    ]
