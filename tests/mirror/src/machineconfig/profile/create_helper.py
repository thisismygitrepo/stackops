...
def test_copy_path_rejects_existing_target_without_overwrite_then_replaces_it(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(FileExistsError, match="overwrite=False"):
        create_helper_module._copy_path(source=source_path, target=target_path, overwrite=False)

    create_helper_module._copy_path(source=source_path, target=target_path, overwrite=True)

    assert target_path.read_text(encoding="utf-8") == "new contents"


def test_copy_path_copies_directory_tree(
    tmp_path: Path,
) -> None:
    ...
    create_helper_module._copy_path(source=source_dir, target=target_dir, overwrite=True)

    assert target_dir.joinpath("nested", "config.txt").read_text(encoding="utf-8") == "payload"


def test_copy_assets_to_machine_for_scripts_copies_all_packaged_files_and_wrap_script(
    tmp_path: Path,
    ...
    assert copied_targets == {
        Path("scripts/bin/alpha.sh"),
        Path("scripts/beta.txt"),
        Path("scripts/wrap_mcfg.nu"),
    }
