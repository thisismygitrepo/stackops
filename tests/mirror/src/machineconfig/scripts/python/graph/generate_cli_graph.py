...
import machineconfig.scripts.python.graph.generate_cli_graph as target
...
def test_module_to_path_resolves_modules_and_packages() -> None:
    module_path = target.module_to_path(
        "machineconfig.scripts.python.graph.generate_cli_graph"
    )
    package_path = target.module_to_path("machineconfig.scripts.python.graph")
