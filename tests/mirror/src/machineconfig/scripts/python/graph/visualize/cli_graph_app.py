...
import machineconfig.scripts.python.graph.visualize.cli_graph_app as target
...
def _install_shell_dependencies(
    ...
    *,
    ...
    abc_module = ModuleType("machineconfig.utils.ssh_utils.abc")
    code_module = ModuleType("machineconfig.utils.code")

    setattr(abc_module, "MACHINECONFIG_VERSION", "machineconfig")
    setattr(
        code_module,
...
def test_tree_runs_render_tree_inside_shell_wrapper(
    ...
    project_dir = str(Path.home().joinpath("code", "machineconfig"))

    rich_tree_module = ModuleType(
        "machineconfig.scripts.python.graph.visualize.rich_tree"
    )
...
def test_dot_writes_output_inside_shell_wrapper(
    ...
    dot_export_module = ModuleType(
        "machineconfig.scripts.python.graph.visualize.dot_export"
    )
...
def test_chart_uses_render_plotly_with_local_project(
    ...
    plotly_views_module = ModuleType(
        "machineconfig.scripts.python.graph.visualize.plotly_views"
    )
    abc_module = ModuleType("machineconfig.utils.ssh_utils.abc")
    project_dir = str(Path.home().joinpath("code/machineconfig"))
...
def test_search_raises_typer_exit_for_non_zero_result(
    ...
    graph_paths_module = ModuleType(
        "machineconfig.scripts.python.graph.visualize.graph_paths"
    )
    search_module = ModuleType(
        "machineconfig.scripts.python.graph.visualize.cli_graph_search"
    )
...
def test_navigate_runs_devops_navigator_in_shell_wrapper(
    ...
    navigator_module = ModuleType(
        "machineconfig.scripts.python.graph.visualize.helpers_navigator.devops_navigator"
    )
