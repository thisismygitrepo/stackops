...
from machineconfig.scripts.python.graph.visualize.helpers_navigator.command_tree import (
    CommandTree,
)
...
def test_build_command_tree_surfaces_load_failures(monkeypatch: MonkeyPatch) -> None:
    tree = CommandTreeHarness("machineconfig commands")

    def explode() -> dict[str, object]:
        raise RuntimeError("boom")

    monkeypatch.setattr(
        "machineconfig.scripts.python.graph.visualize.helpers_navigator.command_tree.load_cli_graph",
        explode,
    )
