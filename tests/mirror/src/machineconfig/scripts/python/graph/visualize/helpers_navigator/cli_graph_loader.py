...
from machineconfig.scripts.python.graph.visualize.helpers_navigator.cli_graph_loader import build_command_nodes, load_cli_graph
...
def test_build_command_nodes_normalizes_commands_arguments_and_usage() -> None:
            ...
            "children": [
                {
                    ...
                    "source": {"module": "machineconfig.graph"},
                    "children": [
                        {
                            "name": "render",
                            "short_help": "Render graph",
                            "help": "Render graph to file",
                            "source": {"dispatches_to": "machineconfig.graph.render"},
                            "signature": {
                                "parameters": [
    ...
    assert group_node.info.module_path == "machineconfig.graph"
    ...
    assert command_node.info.module_path == "machineconfig.graph.render"
    ...
    assert nodes_with_root[0].info.command == "machineconfig graph"
    assert nodes_with_root[0].children[0].info.command == "machineconfig graph render"
