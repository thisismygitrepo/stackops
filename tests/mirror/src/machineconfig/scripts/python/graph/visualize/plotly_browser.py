...
def test_resolve_plotly_browser_path_prefers_valid_environment_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_resolve_plotly_browser_path_rejects_invalid_environment_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_is_supported_browser_path_rejects_snap_wrapper(tmp_path: Path) -> None:
    browser_path = _make_executable_script(tmp_path.joinpath("chromium"), '#!/bin/sh\nexec /snap/bin/chromium "$@"\n')

    assert plotly_browser._is_snap_browser_wrapper(browser_path=browser_path) is True
    assert plotly_browser._is_supported_browser_path(browser_path=browser_path) is False


def test_write_plotly_image_sets_browser_path_for_export(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    browser_path = _make_executable_script(tmp_path.joinpath("chrome"), "#!/bin/sh\nexit 0\n")
...
def test_write_plotly_image_wraps_export_failures(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    browser_path = _make_executable_script(tmp_path.joinpath("chrome"), "#!/bin/sh\nexit 0\n")
    ...
    with pytest.raises(RuntimeError, match="Static Plotly export failed"):
        plotly_browser.write_plotly_image(fig=ExplodingFigure(), output=tmp_path.joinpath("graph.png"), width=640, height=360)
