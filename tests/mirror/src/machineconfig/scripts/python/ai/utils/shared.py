...
def test_get_generic_instructions_path_adjusts_windows_terms(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_get_path_reference_path(_module: object, _path_reference: str) -> Path:
        return source_path

    monkeypatch.setattr(shared_module, "get_path_reference_path", fake_get_path_reference_path)
...
def test_get_generic_instructions_path_preserves_posix_text(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    source_path = tmp_path.joinpath("dev.instructions.md")
    source_path.write_text("bash ./run.sh", encoding="utf-8")
