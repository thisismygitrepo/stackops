...
from machineconfig.scripts.python.ai.solutions.droid.droid import build_configuration
...
def test_build_configuration_writes_droid_markdown_when_enabled(tmp_path: Path) -> None:
    ...
    assert tmp_path.joinpath("DROID.md").read_text(encoding="utf-8") == get_generic_instructions_path().read_text(
        encoding="utf-8"
    )
