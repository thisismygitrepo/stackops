...
from machineconfig.scripts.python.ai.solutions.claude import claude as claude_module


def test_build_configuration_writes_project_and_private_files(
    tmp_path: Path,
    ...
    def fake_adjust_gitignore(
        *,
        repo_root: Path,
        include_default_entries: bool,
    ...
    claude_module.build_configuration(
        repo_root=repo_root,
    ...
    assert (repo_root / ".mcp.json").read_text(encoding="utf-8") == (
        json.dumps({"mcpServers": {}}, indent=2) + "\n"
    )
    assert (repo_root / "CLAUDE.local.md").read_text(encoding="utf-8").startswith(
        "# Local Claude Code preferences"
    )
    assert (repo_root / ".gitignore").is_file()
    assert gitignore_calls == [
        (repo_root, False, [".claude/settings.local.json", "CLAUDE.local.md"])
    ]


def test_build_configuration_preserves_existing_private_files(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo_root = tmp_path / "repo"
