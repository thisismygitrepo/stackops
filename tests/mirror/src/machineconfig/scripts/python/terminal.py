...
from machineconfig.scripts.python import terminal as module
...
def test_attach_to_session_runs_requested_script(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        monkeypatch,
        "machineconfig.scripts.python.helpers.helpers_sessions.attach_impl",
        {"choose_session": lambda **kwargs: ("run_script", "echo launch")},
    )
    _install_module(monkeypatch, "machineconfig.utils.code", {"exit_then_run_shell_script": lambda *, script, strict: calls.append((script, strict))})
...
def test_summarize_parses_comment_json_and_reports_counts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _install_module(monkeypatch, "machineconfig.utils.io", {"remove_c_style_comments": lambda text: text.replace("/* test */", "")})
...
def test_run_aoe_normalizes_optional_lists(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "machineconfig.scripts.python.helpers.helpers_sessions.sessions_cli_run_aoe", {"run_aoe_cli": fake_run_aoe_cli})
