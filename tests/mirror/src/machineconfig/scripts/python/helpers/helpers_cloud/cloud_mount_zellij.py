...
from machineconfig.scripts.python.helpers.helpers_cloud import cloud_mount_zellij
...
def test_build_zellij_layout_kdl_focuses_only_first_tab() -> None:
    layout = cloud_mount_zellij._build_zellij_layout_kdl(
        mount_commands={"alpha": "rclone mount alpha", "beta": "rclone mount beta"}, mount_locations={"alpha": r"C:\alpha", "beta": "/tmp/beta"}
    )
...
def test_build_zellij_launch_command_writes_layout_file() -> None:
    command = cloud_mount_zellij.build_zellij_launch_command(
        mount_commands={"alpha": "rclone mount alpha"}, mount_locations={"alpha": "/tmp/alpha"}, session_name="team sync"
    )
    parts = shlex.split(command)
    layout_path = Path(parts[4])

    try:
        assert parts[:4] == ["zellij", "--session", "team sync", "--new-session-with-layout"]
