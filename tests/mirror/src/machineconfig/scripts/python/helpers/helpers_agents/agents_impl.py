...
import machineconfig.scripts.python.helpers.helpers_agents.agents_impl as agents_impl_module
from machineconfig.scripts.python.helpers.helpers_agents.agents_create_artifacts import CreateArtifactsOutput
from machineconfig.scripts.python.helpers.helpers_agents.agents_create_inputs import ResolvedContextInput, ResolvedPromptInput
...
def test_agents_create_normalizes_codex_provider_and_writes_layout(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("machineconfig.utils.accessories.get_repo_root", lambda _cwd: repo_root)
    monkeypatch.setattr("machineconfig.scripts.python.helpers.helpers_agents.fire_agents_help_launch.prep_agent_launch", fake_prep_agent_launch)
    monkeypatch.setattr(
        "machineconfig.scripts.python.helpers.helpers_agents.fire_agents_help_launch.get_prompt_directories",
        lambda *, prompt_root: [prompt_root / "agent_0"],
    )
    monkeypatch.setattr(
        "machineconfig.scripts.python.helpers.helpers_agents.fire_agents_help_launch.get_agents_launch_layout",
