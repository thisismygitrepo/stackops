...
import machineconfig.scripts.python.helpers.helpers_sessions.sessions_aoe_impl as subject
...
def test_build_modern_aoe_add_command_includes_provider_and_override() -> None:
    ...
    command = subject._build_modern_aoe_add_command(
        tab=tab,
        title="main",
        group="group-a",
        options=options,
    ...
    assert command[13] == (
        "--model gpt-5.4 --config 'model_provider=\"openai\"' "
        "--sandbox workspace-write --debug"
    )
...
def test_build_modern_aoe_add_command_rejects_provider_for_non_codex_agent() -> None:
    ...
    with pytest.raises(ValueError, match="--provider"):
        subject._build_modern_aoe_add_command(
            tab=tab,
            title="main",
            group="group-a",
            options=options,
...
def test_inspect_aoe_add_interface_detects_modern(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args=args[0],
            returncode=0,
            stdout="usage: aoe add --cmd-override",
            stderr="",
        ),
    )
...
def test_inspect_aoe_add_interface_raises_on_failure(
...
+
+
+def test_run_layouts_via_aoe_dry_run_prints_commands_and_launches(
+    monkeypatch: pytest.MonkeyPatch,
...
+        interface: subject.AoeAddInterface,
+    ) -> list[str]:
+        _ = tab, interface
...
+
+
+def test_run_layouts_via_aoe_raises_when_add_command_fails(
+    monkeypatch: pytest.MonkeyPatch,
+) -> None:
+    monkeypatch.setattr(subject, "_command_exists", lambda command: True)
+    monkeypatch.setattr(
+        subject,
+        "_inspect_aoe_add_interface",
+        lambda aoe_bin: subject.AoeAddInterface(style="legacy"),
+    )
+    monkeypatch.setattr(
+        subject.subprocess,
+        "run",
