...
def test_resolve_layouts_file_returns_existing_explicit_path(
    ...
    monkeypatch.setattr(
        subject,
        "find_layout_file",
        lambda *, layout_path: str(layouts_file),
    )
...
def test_resolve_layouts_file_missing_emits_help_and_exits(
    ...
    monkeypatch.setattr(
        subject,
        "find_layout_file",
        lambda *, layout_path: str(missing),
    )
...
@pytest.mark.parametrize(
    ("choose_layouts", "expected_names", "interactive"),
    [
        (None, [], False),
        ("", [], True),
        ("api, web", ["api", "web"], False),
    ],
)
...
+
+    assert captured == {
+        "layouts_json_file": "/tmp/layouts.json",
+        "selected_layouts_names": expected_names,
+        "select_interactively": interactive,
+    }
+
+
+def test_choose_tabs_from_layouts_selects_allowed_tabs_only(
+    monkeypatch: pytest.MonkeyPatch,
+) -> None:
...
+
+    selected = subject.choose_tabs_from_layouts(
...
+            "layoutTabs": [alpha_shared, beta_build],
+        }
+    ]
+
+
+def test_choose_tabs_from_layouts_rejects_unknown_selector(
+    monkeypatch: pytest.MonkeyPatch,
+) -> None:
+    layouts_selected: list[LayoutConfig] = [
+        {
...
+        subject.choose_tabs_from_layouts(
+            Path("/tmp/layouts.json"),
+            layouts_selected,
+            "missing",
+        )
+
+
+def test_substitute_home_in_layouts_preserves_layout_names(
+    monkeypatch: pytest.MonkeyPatch,
+) -> None:
+    layouts_selected: list[LayoutConfig] = [
+        {
+            "layoutName": "alpha",
+            "layoutTabs": [{"tabName": "shared", "startDir": "~", "command": "echo a"}],
+        }
+    ]
+
...
+        {
+            "layoutName": "alpha",
+            "layoutTabs": [{"tabName": "shared", "startDir": "/resolved/0", "command": "echo a"}],
+        }
+    ]
+
+
...
+
+
+def test_resolve_standard_backend_rejects_tmux_on_windows(
+    monkeypatch: pytest.MonkeyPatch,
+    capsys: pytest.CaptureFixture[str],
+) -> None:
