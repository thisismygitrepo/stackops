...
def test_resolve_layout_source_uses_generated_test_layouts(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    generated_layouts = [{"layoutName": "alpha", "layoutTabs": []}, {"layoutName": "beta", "layoutTabs": []}]
    ...
    layout_source = subject.resolve_layout_source(ctx=make_context("HELP"), layouts_file=None, test_layout=True)

    assert layout_source == subject.LayoutSource(source_label="generated test layout", all_layouts=generated_layouts, is_test_layout=True)
...
def test_resolve_layout_source_rejects_mixed_sources() -> None:
    with pytest.raises(ValueError, match="cannot be used together"):
        subject.resolve_layout_source(ctx=make_context("HELP"), layouts_file="layouts.json", test_layout=True)
...
def test_resolve_layouts_file_path_missing_shows_help_and_exits(
    ...
    with pytest.raises(typer.Exit) as exc_info:
        subject._resolve_layouts_file_path(ctx=make_context("HELP TEXT"), layouts_file=None)
...
def test_load_selected_layouts_and_choose_tabs_clone_and_filter() -> None:
    layout_source = subject.LayoutSource(
        ...
        all_layouts=[
            {"layoutName": "Alpha", "layoutTabs": [{"tabName": "tab-1", "startDir": ".", "command": "echo alpha"}]},
            {
                "layoutName": "Beta",
                "layoutTabs": [
    ...
    selected_layouts = subject.load_selected_layouts_from_source(layout_source=layout_source, choose_layouts="alpha")
    ...
    custom_layouts = subject.choose_tabs_from_source(
        layout_source=layout_source, layouts_selected=[layout_source.all_layouts[1]], choose_tabs="tab-1,Beta::tab-2"
    )
    ...
    with pytest.raises(ValueError, match="matched no tabs"):
        subject.choose_tabs_from_source(layout_source=layout_source, layouts_selected=[layout_source.all_layouts[1]], choose_tabs="missing-tab")
