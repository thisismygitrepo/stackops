...
def test_select_layout_strips_comments_and_matches_case_insensitively(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    layouts_file = tmp_path / "layouts.json"
    layouts_file.write_text(
        """/*comment*/{"layouts":[{"layoutName":"Alpha","layoutTabs":[{"tabName":"one","startDir":".","command":"echo 1"}]}]}""", encoding="utf-8"
    )
    ...
    io_module.remove_c_style_comments = remove_c_style_comments
    install_module(monkeypatch, "machineconfig.utils.io", io_module)

    selected = subject.select_layout(layouts_json_file=str(layouts_file), selected_layouts_names=["alpha"], select_interactively=False)

    assert [layout["layoutName"] for layout in selected] == ["Alpha"]


def test_find_layout_file_returns_match_for_missing_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_resolve_conflicts_for_batch_skips_restarts_and_renames(monkeypatch: pytest.MonkeyPatch) -> None:
    layouts_batch = [
    ...
    def fake_build_session_launch_plan(
        requested_session_names: list[str], backend: subject.BackendName, on_conflict: SessionConflictAction
    ...
    layouts_to_run, sessions_to_restart = subject.resolve_conflicts_for_batch(
        layouts_batch=layouts_batch, backend="windows-terminal", on_conflict=cast(SessionConflictAction, "rename")
    )
...
def test_run_layouts_windows_terminal_batches_and_restarts(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    layouts_selected = [{"layoutName": "One", "layoutTabs": []}, {"layoutName": "Two", "layoutTabs": []}]
    ...
    def fake_resolve_conflicts_for_batch(
        layouts_batch: list[dict[str, object]], backend: subject.BackendName, on_conflict: SessionConflictAction
    ) -> tuple[list[dict[str, object]], set[str]]:
        resolve_calls.append([cast(str, layout["layoutName"]) for layout in layouts_batch])
        assert backend == "windows-terminal"
        assert on_conflict == cast(SessionConflictAction, "rename")
    ...
    class FakeWTLocalManager:
        def __init__(self, session_layouts: list[dict[str, object]], session_name_prefix: str | None, exit_mode: SessionExitMode) -> None:
            assert session_name_prefix is None
            assert exit_mode == cast(SessionExitMode, "stayOpen")
            initialized_batches.append([cast(str, layout["layoutName"]) for layout in session_layouts])
    ...
    wt_module.WTLocalManager = FakeWTLocalManager
    install_module(monkeypatch, "machineconfig.cluster.sessions_managers.windows_terminal.wt_local_manager", wt_module)
    ...
    monkeypatch.setattr(time_module, "sleep", lambda seconds: slept_seconds.append(seconds))
