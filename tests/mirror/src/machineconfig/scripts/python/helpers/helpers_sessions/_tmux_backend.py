...
from machineconfig.scripts.python.helpers.helpers_sessions import _tmux_backend as tmux_backend
...
def test_choose_session_window_returns_error_for_unknown_target(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_build_window_target_options(
        ...
        classify_pane_status_fn: object,
        quote_fn: object,
        ...
        return (
            {
                "[main] 1:code": "ATTACH-CODE",
                "[main] 2:logs": "ATTACH-LOGS",
            },
            {
                "[main] 1:code": "preview-code",
                "[main] 2:logs": "preview-logs",
            },
        )
...
def test_choose_kill_target_window_deduplicates_selected_scripts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_build_kill_target_options(
        ...
        return (
            {f"[{session_name}] 1:work.0 shell": f"PANE {session_name}"},
            {f"[{session_name}] 1:work.0 shell": f"preview:{session_name}"},
        )
