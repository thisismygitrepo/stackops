...
def test_kill_script_for_target_handles_session_window_and_pane() -> None:
    ...
    assert (
        tmux_options.kill_script_for_target(session_name="work", quote_fn=_quote, window_target="2")
        == "tmux kill-window -t <work:2>"
    )
    assert (
        tmux_options.kill_script_for_target(
            session_name="work",
            quote_fn=_quote,
            window_target="2",
            pane_index="1",
        )
        == "tmux kill-pane -t <work:2.1>"
    )


def test_build_window_target_options_builds_labels_previews_and_skips_missing_sessions(
    ...
    assert previews == {
        "[alpha] 1:editor *": "WINDOW alpha:1",
        "[alpha] 1:editor.0 shell *": "PANE alpha:1.0",
    }
