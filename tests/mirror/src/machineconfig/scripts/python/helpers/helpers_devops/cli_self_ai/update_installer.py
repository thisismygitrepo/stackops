...
import machineconfig.scripts.python.helpers.helpers_devops.cli_self_ai.update_installer as update_installer_module


def test_get_update_installer_prompt_mentions_resolved_target_file(monkeypatch, tmp_path: Path) -> None:
    expected_relative_path = Path("src", "machineconfig", "jobs", "installer", "installer_data.json")
    ...
    prompt = update_installer_module._get_update_installer_prompt()
...
def test_update_installer_uses_default_prompt_and_context_path(monkeypatch, tmp_path: Path) -> None:
    repo_root = tmp_path
    repo_root.joinpath("pyproject.toml").write_text("", encoding="utf-8")
    relative_path = Path("src", "machineconfig", "jobs", "installer", "installer_data.json")
