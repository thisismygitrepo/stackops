...
_INVALID_BACKUP_CONFIGS: list[tuple[dict[str, object], str]] = [
    (
        {"group": {"item": {"path": "~/.bashrc", "path_cloud": "^", "zip": True, "encrypt": True, "rel2home": True, "os": ["linux"]}}},
...
def test_parse_backup_config_rejects_invalid_fields(raw_config: dict[str, object], message: str) -> None:
    with pytest.raises(ValueError, match=re.escape(message)):
        backup_config_module._parse_backup_config(raw_config)


def test_load_backup_config_returns_none_for_empty_and_non_mapping_yaml(tmp_path: Path) -> None:
...
def test_write_and_read_backup_config_round_trip_and_merge_all(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    library_config: backup_config_module.BackupConfig = {
        "dotfiles": {"bash": {"path_local": "~/.bashrc", "path_cloud": "^", "zip": True, "encrypt": False, "rel2home": True, "os": {"linux"}}}
    }
    user_config: backup_config_module.BackupConfig = {
        "secrets": {
            "tokens": {"path_local": "~/secrets", "path_cloud": None, "zip": False, "encrypt": True, "rel2home": True, "os": {"linux", "darwin"}}
        }
    }
    ...
    assert backup_config_module.read_backup_config(repo="all") == {**library_config, **user_config}
    ...
    assert serialized_user_config.index("- linux") < serialized_user_config.index("- darwin")


def test_describe_missing_backup_config_reports_precise_path_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "does not exist" in backup_config_module.describe_missing_backup_config(repo="user")

    directory_user_path.mkdir()
    monkeypatch.setattr(backup_config_module, "USER_BACKUP_PATH", directory_user_path)
    assert "not a file" in backup_config_module.describe_missing_backup_config(repo="user")

    invalid_library_path.write_text("[]\n", encoding="utf-8")
    monkeypatch.setattr(backup_config_module, "LIBRARY_BACKUP_PATH", invalid_library_path)
    assert "empty or invalid" in backup_config_module.describe_missing_backup_config(repo="library")
