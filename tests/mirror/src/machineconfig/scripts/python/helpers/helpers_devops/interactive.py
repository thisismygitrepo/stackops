...
from machineconfig.scripts.python.helpers.helpers_devops import interactive as module_under_test
...
def test_execute_installations_runs_cli_group_installer_and_reloads_linux_shell(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    install_stub_module(monkeypatch, "machineconfig.utils.installer_utils.installer_cli", {"main_installer_cli": fake_main_installer_cli})
...
def test_main_executes_selected_steps_then_reloads_shell_profile(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    install_stub_module(
        monkeypatch, "machineconfig.profile.create_shell_profile", {"reload_shell_profile_and_exit": lambda: reload_calls.append("reloaded")}
    )
    monkeypatch.setattr(module_under_test.console, "print", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(module_under_test, "display_header", lambda: None)
    monkeypatch.setattr(module_under_test, "get_installation_choices", lambda: ["install_machineconfig"])
    ...
    assert execute_calls == [["install_machineconfig"]]
