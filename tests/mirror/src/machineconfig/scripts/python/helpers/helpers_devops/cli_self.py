...
def test_update_runs_shell_script_and_copies_assets_on_linux(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert "uv tool install --no-cache --upgrade --editable $HOME/code/machineconfig" in scripts[0]
...
def test_install_dev_clones_repo_before_running_uv_install(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    expected_repo_path = home_path.joinpath("code", "machineconfig")

    assert clone_calls == [("https://github.com/thisismygitrepo/machineconfig.git", str(expected_repo_path))]
    assert len(scripts) == 1
...
def test_build_docker_raises_when_script_missing(
    ...
    monkeypatch.setitem(sys.modules, "machineconfig", fake_module)
