...
from machineconfig.scripts.python import utils as module
...
def test_inspect_utils_help_emojis_delegates_to_devops(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "machineconfig.scripts.python.devops", {"emoji_display_diagnostics": fake_emoji_display_diagnostics})
...
def test_get_app_help_lists_expected_subapps(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "machineconfig.scripts.python.helpers.helpers_utils.file_utils_app", {"get_app": lambda: fake_app})
    _install_module(monkeypatch, "machineconfig.scripts.python.helpers.helpers_utils.machine_utils_app", {"get_app": lambda: fake_app})
    _install_module(monkeypatch, "machineconfig.scripts.python.helpers.helpers_utils.pyproject_utils_app", {"get_app": lambda: fake_app})
    runner = CliRunner()
