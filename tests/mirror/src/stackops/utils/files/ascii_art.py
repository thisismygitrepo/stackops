...
from stackops.utils.files import ascii_art
...
def test_get_art_falls_back_to_stackops_when_fortune_is_unavailable(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[str] = []
    ...
    assert calls[1].startswith('figlet -f Banner "stackops" | boxes -d shell')
    assert result == "> croco\n"
