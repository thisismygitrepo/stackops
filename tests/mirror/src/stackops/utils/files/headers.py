...
from stackops.utils.files import ascii_art, headers
from stackops.utils.installer_utils import installer_locator_utils, installer_runner
...
def test_print_header_renders_shell_panel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(installer_runner, "get_stackops_version", lambda: "9.9.9")

    headers.print_header()

    assert len(printed) == 1
    panel = printed[0]
    assert getattr(panel, "title", None) == "[bold blue]✨ 🐊 StackOps Shell 9.9.9 ✨ Made with 🐍 | Built with ❤️[/bold blue]"
