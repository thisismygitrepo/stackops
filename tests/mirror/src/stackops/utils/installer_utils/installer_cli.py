...
def test_build_installer_option_to_data_rejects_duplicate_labels(
    ...
    monkeypatch.setattr("stackops.utils.installer_utils.installer_class.Installer", DuplicateLabelInstaller)
...
def test_build_interactive_option_previews_includes_groups_and_installers(
    ...
    monkeypatch.setattr(
        "stackops.jobs.installer.package_groups.PACKAGE_GROUP2NAMES",
        {"tools": ["alpha", "beta"]},
    )
...
def test_install_clis_routes_urls_and_named_installers(
    ...
    monkeypatch.setattr("stackops.utils.installer_utils.install_from_url.install_from_github_url", fake_github_install)
    monkeypatch.setattr("stackops.utils.installer_utils.install_from_url.install_from_binary_url", fake_binary_install)
    monkeypatch.setattr(
        "stackops.utils.installer_utils.installer_runner.get_installers",
        lambda os, arch, which_cats: [_make_installer_data("Alpha")],
    )
    monkeypatch.setattr(
        "stackops.utils.installer_utils.installer_summary.render_installation_summary",
        fake_render_installation_summary,
    )
    monkeypatch.setattr("stackops.utils.installer_utils.installer_class.Installer", RuntimeInstaller)
