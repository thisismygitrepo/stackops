...
""",
    ...
    def fetch_dates(repositories_by_key: dict[str, str], console: Console) -> dict[str, datetime]:
        assert repositories_by_key == {"owner/repository": "Owner/Repository"}
        assert console is render_console
        return {"owner/repository": datetime(2026, 8, 15, 12, 30, 45, tzinfo=UTC)}
    ...
    updated_count, repository_count = update_github_commit_dates.update_installer_commit_dates(
    ...
    assert (updated_count, repository_count) == (2, 1)
    ...
    assert "once all GitHub requests succeed" in rendered_output


def test_fetch_failure_leaves_outputs_untouched(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    installer_data_path = tmp_path.joinpath("installer_data.json")
    ...
    def fail_fetch(repositories_by_key: dict[str, str], console: Console) -> dict[str, datetime]:
        assert repositories_by_key == {"owner/repository": "owner/repository"}
        assert console is render_console
