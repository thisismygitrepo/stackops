...
from stackops.utils import upgrade_packages as sut
import stackops.utils.code
...
""".strip()
    ...
    script_path.write_text('REQ = "stackops>=8.09"\nEXTRA = "stackops[extra]>=8.09"\n', encoding="utf-8")
    shell_path.write_text('echo "stackops[dev]>=8.09"\n', encoding="utf-8")
    dockerfile_path.write_text('RUN echo "stackops>=8.09"\n', encoding="utf-8")
    recorded_commands: list[str] = []

    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(stackops.utils.code, "exit_then_run_shell_script", lambda command: recorded_commands.append(command))

    sut.upgrade_machine_config_version()

    assert 'version = "8.10"' in pyproject_path.read_text(encoding="utf-8")
    assert "stackops>=8.10" in script_path.read_text(encoding="utf-8")
    assert "stackops[extra]>=8.10" in script_path.read_text(encoding="utf-8")
    assert "stackops[dev]>=8.10" in shell_path.read_text(encoding="utf-8")
    assert "stackops>=8.10" in dockerfile_path.read_text(encoding="utf-8")
