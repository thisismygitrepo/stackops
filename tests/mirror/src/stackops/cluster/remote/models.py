...
import stackops.cluster.remote.models as models_module
from stackops.cluster.remote.models import JobStatus, RemoteStackOpsConfig, WorkloadParams
...
def test_remote_machine_config_requires_cloud_name_for_cloud_transfer() -> None:
    with pytest.raises(ValueError, match="cloud_name must be provided"):
        RemoteStackOpsConfig(transfer_method="cloud")
...
def test_remote_machine_config_uses_default_notification_values(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = RemoteStackOpsConfig(notify_upon_completion=True)
