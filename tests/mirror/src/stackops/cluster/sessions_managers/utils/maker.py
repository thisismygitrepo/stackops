...
def test_get_fire_tab_using_uv_wraps_named_function(
    ...
    fake_meta = ModuleType("stackops.utils.meta")
    setattr(fake_meta, "lambda_to_python_script", fake_lambda_to_python_script)
    fake_code = ModuleType("stackops.utils.code")
    setattr(fake_code, "get_uv_command_executing_python_script", fake_uv_command)
    monkeypatch.setitem(sys.modules, "stackops.utils.meta", fake_meta)
    monkeypatch.setitem(sys.modules, "stackops.utils.code", fake_code)
...
def test_get_fire_tab_using_fire_builds_linux_command(
    ...
    fake_source_of_truth = ModuleType("stackops.utils.source_of_truth")
