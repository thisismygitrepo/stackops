...
from stackops.profile import create_links as create_links_module
from stackops.profile import create_links_export as create_links_export_module
...
def test_main_from_parser_maps_aliases_and_dispatches_selected_items(
...
def test_main_from_parser_exits_when_no_valid_items_are_selected(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_read_mapper(*, repo: create_links_module.RepoLoose) -> create_links_module.MapperFileData:
        assert repo == "library"
        return {
            "public": {"git": [_build_mapper_entry("gitconfig")]},
            "private": {},
        }
