...
import stackops.scripts.python.croshell as croshell_module
...
def test_croshell_uses_stackops_project_path_and_backend_alias(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    home_dir = tmp_path.joinpath("home")
    project_dir = home_dir.joinpath("code", "stackops")
    project_dir.mkdir(parents=True)

    calls: list[Invocation] = []
    fake_module = ModuleType("stackops.scripts.python.helpers.helpers_croshell.croshell_impl")
    ...
    monkeypatch.setitem(sys.modules, "stackops.scripts.python.helpers.helpers_croshell.croshell_impl", fake_module)
    monkeypatch.setattr(Path, "home", lambda: home_dir)

    croshell_module.croshell(path="script.py", project_path=None, uv_with="rich", backend="i", profile="dev", stackops_project=True, frozen=True)
