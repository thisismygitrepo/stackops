...
from stackops.scripts.python.helpers.helpers_utils import pyproject_utils_app as module
...
def test_type_hint_rejects_missing_path(capsys: pytest.CaptureFixture[str]) -> None:
    missing_path = Path("/tmp/stackops-pyproject-utils-app-missing")
...
def test_upgrade_packages_resolves_paths_and_forwards_clean_groups(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _install_module(
        monkeypatch,
        "stackops.utils.upgrade_packages",
...
def test_init_project_delegates_to_helper_impl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "stackops.scripts.python.helpers.helpers_utils.python", {"init_project": fake_init_project})
