...
from stackops.scripts.python.helpers.helpers_utils import python as module
...
def test_choose_with_tv_returns_selected_value(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_module(
        monkeypatch, "stackops.utils.installer_utils.installer_locator_utils", {"check_tool_exists": lambda tool_name: tool_name == "tv"}
    )
    _install_module(monkeypatch, "stackops.utils.options_utils.tv_options", {"choose_from_dict_with_preview": lambda **_: "KEY = value"})
...
def test_edit_file_with_hx_builds_repo_aware_command(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _install_module(monkeypatch, "stackops.utils.accessories", {"get_repo_root": lambda root_path: repo_root})
    _install_module(monkeypatch, "stackops.utils.code", {"exit_then_run_shell_script": lambda script: commands.append(script)})
...
def test_get_machine_specs_writes_machine_specs_json(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _install_module(monkeypatch, "stackops.utils.code", {"get_uv_command": lambda platform: "uv"})
    _install_module(monkeypatch, "stackops.utils.source_of_truth", {"CONFIG_ROOT": config_root})
