...
type ParseBackupConfig = Callable[[dict[str, object]], backup_config_module.BackupConfig]
type LoadBackupConfig = Callable[[Path], backup_config_module.BackupConfig | None]

parse_backup_config = cast(
    ParseBackupConfig,
    getattr(backup_config_module, "_parse_backup_config"),
)
load_backup_config = cast(
    LoadBackupConfig,
    getattr(backup_config_module, "_load_backup_config"),
)
...
def test_parse_backup_config_rejects_invalid_fields(
    ...
    with pytest.raises(ValueError, match=re.escape(message)):
        parse_backup_config(raw_config)
...
def test_load_backup_config_returns_none_for_empty_and_non_mapping_yaml(
    ...
    assert load_backup_config(missing_path) is None
    assert load_backup_config(empty_path) is None
    assert load_backup_config(sequence_path) is None
