...
def test_subset_cli_which_selects_an_entry_and_a_whole_group_without_opening_picker(
    ...
    def fail_if_picker_opens(*, config: BackupConfig, source_path: Path) -> list[cli_data_subset.BackupEntryKey]:
        raise AssertionError(f"Picker opened for {source_path} with {len(config)} group(s)")

    monkeypatch.setattr(cli_data_subset, "USER_BACKUP_PATH", source_path)
    monkeypatch.setattr(cli_data_subset, "_choose_subset_entry_keys", fail_if_picker_opens)

    result = CliRunner().invoke(cli_data.get_app(), ["subset", str(output_path), "--which", "alpha.two,beta"])
    ...
    assert list(output_config["beta"]) == ["three", "four"]
...
def test_subset_data_file_writes_selected_entries_in_source_order(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cli_data_subset.subset_data_file(source_path=source_path, output_path=output_path, on_conflict="throw-error", which=None)
...
def test_subset_data_file_append_merges_groups_without_replacing_entries(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cli_data_subset.subset_data_file(source_path=source_path, output_path=output_path, on_conflict="append", which=None)
...
def test_subset_data_file_refuses_source_as_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(typer.Exit) as exc_info:
        cli_data_subset.subset_data_file(source_path=source_path, output_path=source_path, on_conflict="overwrite", which=None)
