...
from stackops.scripts.python.helpers.helpers_devops import cli_ssh as module
...
def install_helpers_network_ssh_submodule(
    ...
    ssh_module = ModuleType("stackops.scripts.python.helpers.helpers_network.ssh")
    ...
    monkeypatch.setitem(sys.modules, "stackops.scripts.python.helpers.helpers_network.ssh", ssh_module)
    monkeypatch.setitem(sys.modules, f"stackops.scripts.python.helpers.helpers_network.ssh.{leaf_name}", leaf_module)
...
def test_install_ssh_server_runs_linux_script(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(sys.modules, "stackops.utils.code", code_module)
...
def test_change_ssh_port_forwards_to_helper(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    helper_module = ModuleType("stackops.utils.ssh_utils.wsl")
    ...
    monkeypatch.setitem(sys.modules, "stackops.utils.ssh_utils.wsl", helper_module)
    monkeypatch.setattr(platform, "system", lambda: "Linux")
...
def test_add_ssh_key_forwards_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    helper_module = ModuleType("stackops.scripts.python.helpers.helpers_network.ssh.ssh_add_ssh_key")
...
def test_debug_ssh_dispatches_to_linux_helper(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    helper_module = ModuleType("stackops.scripts.python.helpers.helpers_network.ssh.ssh_debug_linux")
