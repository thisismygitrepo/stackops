...
from stackops.scripts.python.helpers.helpers_devops import devops_status_checks as module


def install_profile_create_shell_profile_module(monkeypatch: pytest.MonkeyPatch, profile_path: Path) -> None:
    import stackops.profile as profile_package

    profile_module = ModuleType("stackops.profile.create_shell_profile")

    def fake_get_shell_profile_path() -> Path:
        return profile_path

    setattr(profile_module, "get_shell_profile_path", fake_get_shell_profile_path)
    monkeypatch.setitem(sys.modules, "stackops.profile.create_shell_profile", profile_module)
...
def test_check_repos_status_reports_missing_and_git_repos(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    io_module = ModuleType("stackops.utils.io")
    ...
    monkeypatch.setitem(sys.modules, "stackops.utils.io", io_module)
...
def test_check_config_files_status_aggregates_link_counts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    links_module = ModuleType("stackops.profile.create_links")
    ...
    monkeypatch.setitem(sys.modules, "stackops.profile.create_links", links_module)
...
def test_check_important_tools_uses_shutil_which(monkeypatch: pytest.MonkeyPatch) -> None:
    package_groups_module = ModuleType("stackops.jobs.installer.package_groups")

    def fake_which(name: str) -> str | None:
        return "/usr/bin/git" if name == "git" else None

    setattr(package_groups_module, "PACKAGE_GROUP2NAMES", {"core_tools": ("git", "fd")})
    monkeypatch.setitem(sys.modules, "stackops.jobs.installer.package_groups", package_groups_module)
...
def test_check_backup_config_collects_named_entries(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    io_module = ModuleType("stackops.utils.io")
    backup_module = ModuleType("stackops.scripts.python.helpers.helpers_devops.backup_config")
    ...
    monkeypatch.setitem(sys.modules, "stackops.utils.io", io_module)
    monkeypatch.setitem(sys.modules, "stackops.scripts.python.helpers.helpers_devops.backup_config", backup_module)
