...
from stackops.scripts.python.helpers.helpers_agents import mcp_install as module
...
class LoadJsonObjectFn(Protocol):
    def __call__(self, *, path: Path) -> dict[str, object]: ...


class EnsureObjectMemberFn(Protocol):
    def __call__(self, *, root: dict[str, object], key: str, path: Path) -> dict[str, object]: ...


class WriteSettingsWithMcpServersFn(Protocol):
    def __call__(
        self,
        *,
        path: Path,
...
class WriteResolvedServersFn(Protocol):
    def __call__(self, *, path: Path, resolved_servers: tuple[ResolvedMcpServer, ...]) -> None: ...


class ResolvedServerToOpencodeEntryFn(Protocol):
    def __call__(self, *, resolved_server: ResolvedMcpServer) -> dict[str, object]: ...


class ExtractBearerTokenEnvVarFn(Protocol):
    def __call__(self, *, headers: dict[str, str], server_name: str) -> str: ...


ALL_AGENT_VALUES: tuple[AGENTS, ...] = cast(tuple[AGENTS, ...], get_args(AGENTS))
LOAD_JSON_OBJECT = cast(LoadJsonObjectFn, getattr(module, "_load_json_object"))
ENSURE_OBJECT_MEMBER = cast(EnsureObjectMemberFn, getattr(module, "_ensure_object_member"))
WRITE_SETTINGS_WITH_MCP_SERVERS = cast(
    WriteSettingsWithMcpServersFn,
    getattr(module, "_write_settings_with_mcp_servers"),
)
WRITE_PI_MCP_ADAPTER_CONFIG = cast(WriteResolvedServersFn, getattr(module, "_write_pi_mcp_adapter_config"))
WRITE_CODEX_CONFIG = cast(WriteResolvedServersFn, getattr(module, "_write_codex_config"))
WRITE_CLINE_MCP_SETTINGS = cast(WriteResolvedServersFn, getattr(module, "_write_cline_mcp_settings"))
RESOLVED_SERVER_TO_OPENCODE_ENTRY = cast(
    ResolvedServerToOpencodeEntryFn,
    getattr(module, "_resolved_server_to_opencode_entry"),
)
EXTRACT_BEARER_TOKEN_ENV_VAR = cast(
    ExtractBearerTokenEnvVarFn,
    getattr(module, "_extract_bearer_token_env_var"),
)
LOCAL_INSTALL_CASES: tuple[tuple[AGENTS, Path], ...] = (
...
GLOBAL_INSTALL_CASES: tuple[tuple[AGENTS, Path], ...] = (
    ("codex", Path(".codex/config.toml")),
...
INSTALL_DISPATCH_CASES: tuple[tuple[AGENTS, MCP_INSTALL_SCOPE, str, bool | None], ...] = (
...
def test_parse_requested_agents_blank_means_all_and_duplicates_collapse() -> None:
    assert module.parse_requested_agents("") == ALL_AGENT_VALUES
...
def test_install_resolved_mcp_servers_dispatches_to_expected_writer(
    ...
    def fake_resolve_install_path(*, agent: AGENTS, scope: MCP_INSTALL_SCOPE, repo_root: Path | None, home_dir: Path) -> Path:
        return install_path
...
def test_load_json_object_handles_missing_blank_and_comment_json(tmp_path: Path) -> None:
    ...
    assert LOAD_JSON_OBJECT(path=missing_path) == {}
    assert LOAD_JSON_OBJECT(path=blank_path) == {}
    assert LOAD_JSON_OBJECT(path=commented_path) == {"value": 1}


def test_ensure_object_member_creates_missing_object_and_rejects_scalar_values(tmp_path: Path) -> None:
    root: dict[str, object] = {}

    created = ENSURE_OBJECT_MEMBER(root=root, key="mcpServers", path=tmp_path / "settings.json")

    assert created == {}
    assert root["mcpServers"] == {}

    with pytest.raises(ValueError, match="must be a JSON object"):
        ENSURE_OBJECT_MEMBER(
            root={"mcpServers": 1},
            key="mcpServers",
...
def test_write_settings_with_mcp_servers_merges_entries_and_can_enable_mcp(tmp_path: Path) -> None:
    ...
    WRITE_SETTINGS_WITH_MCP_SERVERS(
        path=path,
...
def test_cline_writer_marks_disabled_http_servers_and_opencode_rejects_cwd(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="does not support 'cwd'"):
        RESOLVED_SERVER_TO_OPENCODE_ENTRY(resolved_server=_stdio_server("local", True, "/repo", "local server"))
...
def test_extract_bearer_token_env_var_rejects_unsupported_headers(
    ...
    with pytest.raises(ValueError, match=message):
        EXTRACT_BEARER_TOKEN_ENV_VAR(headers=headers, server_name="remote")
