...
import stackops.scripts.python.helpers.helpers_agents.privacy.configs.droid.droid_privacy as droid_privacy_module
...
def test_secure_droid_cli_merges_existing_settings(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr(droid_privacy_module.Path, "home", lambda: tmp_path)
    config_file = tmp_path / ".factory" / "settings.json"
    config_file.parent.mkdir(parents=True)
    config_file.write_text(json.dumps({"custom": "keep", "telemetry": True}), encoding="utf-8")
...
def test_secure_droid_cli_replaces_invalid_json(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr(droid_privacy_module.Path, "home", lambda: tmp_path)
    config_file = tmp_path / ".factory" / "settings.json"
    config_file.parent.mkdir(parents=True)
