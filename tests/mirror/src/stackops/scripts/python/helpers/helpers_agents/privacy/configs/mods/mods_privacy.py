...
import stackops.scripts.python.helpers.helpers_agents.privacy.configs.mods.mods_privacy as mods_privacy_module
...
def test_secure_mods_config_updates_existing_keys_and_appends_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_secure_mods_config_creates_default_file_when_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert config_file.read_text(encoding="utf-8") == ('no-cache: true\ncache-path: "/dev/null"\ntelemetry: false\n')
