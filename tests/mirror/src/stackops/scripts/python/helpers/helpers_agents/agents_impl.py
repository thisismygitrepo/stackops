...
def test_agents_create_runs_generated_layout_when_requested(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    captured_run_paths: list[Path] = []
    captured_artifact_run_values: list[bool] = []

    monkeypatch.setattr(agents_impl, "show_agents_create_overview", lambda **kwargs: None)
    monkeypatch.setattr(agents_impl, "show_generated_agents_table", lambda **kwargs: None)
    monkeypatch.setattr(agents_impl, "show_created_artifacts_panel", lambda **kwargs: None)
    monkeypatch.setattr("stackops.utils.accessories.get_repo_root", lambda _path: tmp_path)
    monkeypatch.setattr(agents_impl, "resolve_agents_output_dir", lambda repo_root, agents_dir, job_name: (tmp_path / "agents", "job"))
    ...
    monkeypatch.setattr(
        agents_impl,
        "write_create_artifacts",
        lambda **kwargs: captured_artifact_run_values.append(kwargs["run"])
        or _ArtifactsOutput(
            artifacts_dir=tmp_path / "artifacts",
            recreate_script_path=tmp_path / "recreate.sh",
        ),
    ...
    monkeypatch.setattr(
        agents_impl,
        "_run_generated_layout",
        lambda *, layout_output_path: captured_run_paths.append(layout_output_path),
    )
    ...
    assert captured_artifact_run_values == [True]
    assert captured_run_paths == [(tmp_path / "layout.json").resolve()]
