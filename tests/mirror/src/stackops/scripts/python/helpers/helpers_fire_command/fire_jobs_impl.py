...
import stackops.utils.ve as ve_module
from stackops.scripts.python.helpers.helpers_fire_command import file_wrangler as file_wrangler_module
...
def test_handle_marimo_skips_conversion_for_valid_notebook(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert commands == [
        ["uv", "run", "--project", str(repo_root), "--with", "marimo", "marimo", "check", str(choice_file)]
    ]
...
def test_build_final_command_and_apply_modifiers(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        file_wrangler_module,
        "add_to_path",
        lambda path_variable, directory: f"export {path_variable}={directory}",
    )
...
def test_route_builds_command_and_hands_off_to_shell_runner(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def fake_apply_command_modifiers(
        args: FireJobArgs,
        command: str,
        choice_file: Path,
        repo_root: Path | None,
    ...
    monkeypatch.setattr(
        fire_jobs_impl,
        "_choose_function",
        lambda choose_function, function, choice_file, kwargs_dict: "run",
    )
