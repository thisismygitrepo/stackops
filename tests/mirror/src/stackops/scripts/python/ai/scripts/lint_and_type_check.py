...
def _repo_root() -> Path:
    ...
    for parent in current_path.parents:
        if parent.joinpath("pyproject.toml").exists() and parent.joinpath("src", "stackops").exists():
            return parent
    raise AssertionError("repo root not found")


LINT_SCRIPT_PATH = _repo_root() / "src" / "stackops" / "scripts" / "python" / "ai" / "scripts" / "lint_and_type_check.py"
