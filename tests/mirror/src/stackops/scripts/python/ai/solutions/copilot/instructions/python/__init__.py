...
def test_path_references_resolve_to_existing_instruction_assets() -> None:
    dev_instructions_path = get_path_reference_path(
        module=copilot_python_instructions, path_reference=copilot_python_instructions.DEV_INSTRUCTIONS_PATH_REFERENCE
    )
    watch_exec_prompt_path = get_path_reference_path(
        module=copilot_python_instructions, path_reference=copilot_python_instructions.WATCH_EXEC_PROMPT_PATH_REFERENCE
    )
