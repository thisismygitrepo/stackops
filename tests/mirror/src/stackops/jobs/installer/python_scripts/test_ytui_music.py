...
def test_linux_packages_use_apt_on_debian(
    ...
    monkeypatch.setattr(
        ...
        lambda: LinuxDistribution(distribution_id="debian"),
    )
    ...
    assert executed_commands == [
        ...
        (
            ...
            "install",
            "-y",
            *ytui_music.DEBIAN_REQUIRED_PACKAGES,
        ),
        (
            "sudo",
            "env",
            ...
            "install",
            "-y",
            *ytui_music.DEBIAN_OPTIONAL_PACKAGES,
        ),
    ]


@pytest.mark.parametrize("distribution_id", ["fedora", "rhel", "centos", "rocky", "almalinux"])
def test_dnf_distributions_fail_before_running_commands(
    monkeypatch: pytest.MonkeyPatch,
    distribution_id: str,
) -> None:
    monkeypatch.setattr(
