...
def test_upload_and_download_use_console_output(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("stackops.jobs.installer.checks.install_utils.upload_app", lambda path: "https://example.test/share")
    security_cli.upload(Path("/tmp/tool"))
    assert fake_console.messages[-1] == "https://example.test/share"

    monkeypatch.setattr("stackops.jobs.installer.checks.install_utils.upload_app", lambda path: None)
    with pytest.raises(typer.Exit) as exit_info:
        security_cli.upload(Path("/tmp/tool"))
    assert exit_info.value.exit_code == 1

    monkeypatch.setattr("stackops.jobs.installer.checks.install_utils.download_google_drive_file", lambda url: Path("/tmp/downloaded"))
