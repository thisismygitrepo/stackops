...
def _build_powershell_script(command: str, exit_mode: SessionExitMode) -> str:
    ...
    run_block = "\n".join(
        (
            ...
            f"$stackopsCommand = {command_literal}",
            "Invoke-Expression -Command $stackopsCommand",
            "$stackopsExitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } elseif ($?) { 0 } else { 1 }",
        )
    )
    match exit_mode:
        case "backToShell":
        ...
        case "terminate":
            ...
            return "\n".join(
                (
                    "while ($true) {",
                    indented_run_block,
                    '    $stackopsReply = Read-Host -Prompt ("Process completed with exit code {0}. Press Enter to restart, or type anything and press Enter to close" -f $stackopsExitCode)',
                    '    if ($stackopsReply -ne "") {',
        ...
        case "killWindow":
            return "\n".join((run_block, "exit $stackopsExitCode"))
