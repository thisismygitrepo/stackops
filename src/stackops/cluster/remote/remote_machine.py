...
from stackops.cluster.remote.data_transfer import transfer_sftp, transfer_cloud

console = Console()


class RemoteMachine:
    def __init__(self, func: str | Path | Callable[..., object], config: RemoteStackOpsConfig, func_kwargs: dict[str, object] | None, data: list[str] | None) -> None:
        self.config = config
    ...
    def from_dict(d: dict[str, object]) -> "RemoteMachine":
        ...
        rm.config = RemoteStackOpsConfig.from_dict(config_data)
