# stackops Nushell initialization
...
export def wrap_in_shell_script [command: string, ...args: string] {
    let op_dir = ((get_home) | path join "tmp_results" "tmp_scripts" "stackops")
    ...
    print $"stackops: running ($command) at ($timestamp)"
    print $"stackops: op program path ($op_program_path)"
    ...
    print $"stackops: completed ($command)"
}
