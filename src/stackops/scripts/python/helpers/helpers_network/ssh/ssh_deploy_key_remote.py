...
def deploy_key_to_remote(remote_target: str, pubkey_path: Path, password: str | None) -> bool:
    ...
    console.print(
        Panel(
            f"🔐 Deploying key to remote: [cyan]{remote_target}[/cyan]\n📄 Key: [green]{pubkey_path}[/green]",
    ...
    try:
        ...
        if result:
            console.print(
                Panel(
                    f"✅ Key successfully deployed to [green]{ssh.get_remote_repr(add_machine=True)}[/green]",
...
'''
    resp = ssh.run_shell_cmd_on_remote(
        command=check_cmd, verbose_output=False, description="Checking existing keys", strict_stderr=False, strict_return_code=False
    )
...
'''
    ...
    resp = ssh.run_shell_cmd_on_remote(
        command=deploy_cmd, verbose_output=False, description="Deploying SSH key", strict_stderr=False, strict_return_code=False
    )
...
def _attempt_sshd_restart_unix(ssh: SSH) -> None:
    ...
    restart_commands = [
        "sudo systemctl restart sshd 2>/dev/null || sudo systemctl restart ssh 2>/dev/null || sudo service ssh restart 2>/dev/null || sudo launchctl kickstart -k system/com.openssh.sshd 2>/dev/null || echo 'RESTART_SKIPPED'"
    ]

    resp = ssh.run_shell_cmd_on_remote(
        command=restart_commands[0], verbose_output=False, description="Restarting SSH", strict_stderr=False, strict_return_code=False
    )
...
'''
    ...
    resp = ssh.run_py_remotely(
        python_code=python_code,
        uv_with=None,
        uv_project_dir=None,
        description="Deploying SSH key to Windows",
...
def deploy_multiple_keys_to_remote(remote_target: str, pubkey_paths: list[Path], password: str | None) -> bool:
    ...
    console.print(
        Panel(
            f"🔐 Deploying {len(pubkey_paths)} key(s) to remote: [cyan]{remote_target}[/cyan]",
            title="[bold blue]Remote SSH Key Deployment[/bold blue]",
            border_style="blue",
        )
    )
    ...
    try:
        ...
        if success_count == len(all_keys_content):
            console.print(
                Panel(
                    f"✅ All {success_count} key(s) deployed to [green]{ssh.get_remote_repr(add_machine=True)}[/green]",
                    title="[bold green]Success[/bold green]",
                    border_style="green",
                    box=box.DOUBLE_EDGE,
                )
        ...
        console.print(
            Panel(
                f"⚠️  Deployed {success_count}/{len(all_keys_content)} keys",
