...
def _select_option_spec(*, default: OptionValue, options: list[OptionValue]) -> TextualSelectOptionSpec:
    return {"kind": "select", "default": default, "options": options}


def _text_option_spec(*, default: str | None, allow_blank: bool, placeholder: str) -> TextualTextOptionSpec:
    return {"kind": "text", "default": default, "allow_blank": allow_blank, "placeholder": placeholder}
    ...
    *,
    ...
    return {
        ...
        review_options["agents_dir"]: _text_option_spec(
            default=agents_dir, allow_blank=True, placeholder="Leave blank to derive .ai/agents from job_name"
        ),
        review_options["host"]: _select_option_spec(
            default=host, options=_to_option_values(order_current_first(options=cast(tuple[HOST, ...], get_args(HOST)), current=host))
        ),
        ...
        review_options["provider"]: _select_option_spec(default=provider, options=provider_form_options),
    }
...
def _require_non_negative_float_value(*, selected_values: SelectedOptionMap, key: str) -> float:
    value = _require_selected_value(selected_values=selected_values, key=key)
    if not isinstance(value, str):
        raise TypeError(f"""Expected "{key}" to be str, got {value!r}.""")
    try:
        ...
        raise TypeError(f"""Expected "{key}" to be a non-negative number, got {value!r}.""") from exc
    if not isfinite(parsed) or parsed < 0:
        raise TypeError(f"""Expected "{key}" to be a non-negative number, got {value!r}.""")
    return parsed
