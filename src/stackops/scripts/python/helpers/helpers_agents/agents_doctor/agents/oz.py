...
def collect(*, context: DoctorContext) -> tuple[DoctorResource, ...]:
    configurations = (
        DoctorPathCandidate("mcp.json", "global", context.xdg_config_directory / "stackops/agents/oz/mcp.json", "Oz user MCP configuration", True),
        DoctorPathCandidate("mcp.json", "local", context.project_root / ".warp/mcp.json", "Oz project MCP configuration", True),
    )
    instructions = (DoctorPathCandidate("AGENTS.md", "local", context.project_root / "AGENTS.md", "Oz project guidance", True),)
    skill_roots = (
