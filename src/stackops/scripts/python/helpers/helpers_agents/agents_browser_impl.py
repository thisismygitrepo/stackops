...
def install_agent_browser_skill(*, agent: AGENTS, backend: SKILL_INSTALL_COMMAND_BACKEND) -> BrowserSkillInstallResult:
    ...
    main_installer_cli(which=AGENT_BROWSER_INSTALLER_NAME, group=False, interactive=False, explore=False, update=True, version=None, ctx=None)
...
def install_browser_use_skill(*, agent: AGENTS) -> BrowserSkillInstallResult:
    install_root = BROWSER_TECH_ROOT.expanduser().joinpath(BROWSER_USE_INSTALLER_NAME)
    install_root.mkdir(parents=True, exist_ok=True)

    from stackops.utils.installer_utils.installer_cli import main_installer_cli

    main_installer_cli(which=BROWSER_USE_INSTALLER_NAME, group=False, interactive=False, explore=False, update=True, version=None, ctx=None)
    skill_target = BROWSER_USE_SKILL_TARGET_BY_STACKOPS_AGENT.get(agent, "agents")
    command = (BROWSER_USE_INSTALLER_NAME, "skill", "install", "--target", skill_target, "--no-install")
    _run_required_command(command=command, cwd=install_root)
    return BrowserSkillInstallResult(install_root=install_root, command=command)
...
def install_pinchtab_skill(*, agent: AGENTS, backend: SKILL_INSTALL_COMMAND_BACKEND) -> BrowserSkillInstallResult:
    ...
    main_installer_cli(which=PINCHTAB_INSTALLER_NAME, group=False, interactive=False, explore=False, update=True, version=None, ctx=None)
    command = (backend, SKILLS_CLI_PACKAGE, "add", PINCHTAB_SKILL_REPO, "--skill", PINCHTAB_SKILL_NAME, "--agent", skills_cli_agent, "--yes")
    _run_required_command(command=command, cwd=install_root)
    return BrowserSkillInstallResult(install_root=install_root, command=command)
    ...
    match which:
        case "agent-browser":
            ...
            return BrowserTechInstallResult(
                which=which, install_root=result.install_root, commands=(result.command,), guide_paths=guide_paths, mcp_servers=()
            )
        case "browser-use":
            result = install_browser_use_skill(agent=agent)
            guide_paths = write_browser_tech_files(which=which, install_root=result.install_root)
            return BrowserTechInstallResult(
                which=which, install_root=result.install_root, commands=(result.command,), guide_paths=guide_paths, mcp_servers=()
            )
        ...
        case "playwright-cli":
            ...
            return BrowserTechInstallResult(which=which, install_root=install_root, commands=commands, guide_paths=guide_paths, mcp_servers=())
        case "chrome-devtools-mcp" | "playwright-mcp":
            install_root = BROWSER_MCP_ROOT.expanduser().joinpath(which)
            guide_paths = write_browser_tech_files(which=which, install_root=install_root)
            return BrowserTechInstallResult(
                which=which, install_root=install_root, commands=(), guide_paths=guide_paths, mcp_servers=get_browser_tech_mcp_servers(which=which)
            )
        case _:
            assert_never(which)
