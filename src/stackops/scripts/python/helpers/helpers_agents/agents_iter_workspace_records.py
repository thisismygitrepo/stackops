...
def resolve_iter_workspace_run_path(*, snapshot: HerdrSnapshot, workspace: HerdrWorkspace) -> Path:
    ...
    for agent in workspace_agents:
        for field_name, raw_path in (("cwd", agent.cwd), ("foreground_cwd", agent.foreground_cwd)):
            ...
            for project_root in (resolved_candidate, *resolved_candidate.parents):
                agent_ops_root = project_root.joinpath(".ai", "agent-ops")
                iterations_root = agent_ops_root.joinpath("iterations")
                ...
                if any(path.is_symlink() for path in (project_root.joinpath(".ai"), agent_ops_root, iterations_root, run_path)):
                    raise RuntimeError(f"Refusing symlinked Agent-Ops record ancestry: {run_path}")
    ...
    if len(run_paths) > 1:
        ...
        raise RuntimeError(f"Herdr iter workspace {workspace_identity} resolves to multiple Agent-Ops runs: {paths}")
    ...
    raise RuntimeError(f"Cannot locate Agent-Ops records for Herdr iter workspace {workspace_identity}. Searched from: {searched}.{unusable}")
