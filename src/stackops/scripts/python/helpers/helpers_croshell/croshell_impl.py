...
def _build_fire_line(
    ...
    if backend == "marimo":
        if Path.home().joinpath("code/stackops").exists():
            ...
            requirements = f"""{uv_python_line} {user_uv_with_line} {uv_project_line} --with "marimo,sqlglot,cowsay,stackops[plot]>=8.101" """
        return f"""
...
"""
    ...
    if backend == "jupyter":
        if Path.home().joinpath("code/stackops").exists():
            ...
            requirements = f"""{user_uv_with_line} {uv_project_line} --with "cowsay,stackops[plot]>=8.101" """
...
uv venv
uv add "cowsay,stackops[plot]>=8.101"
uv add {user_uv_add}
# code serve-web
...
"""
    ...
    if Path.home().joinpath("code/stackops").exists():
        ...
        ve_line = f"""{uv_python_line} {user_uv_with_line} {uv_project_line} --with "cowsay,stackops[plot]>=8.101" """
