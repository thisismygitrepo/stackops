...
def _different_detail(snapshot: RepositorySnapshot, current: RepositorySnapshot) -> str:
    ...
    if snapshot["sync"] != current["sync"]:
        differences.append("repository sync settings changed")
    if snapshot["sync"]["mode"] == current["sync"]["mode"] == "git" and snapshot["remotes"] != current["remotes"]:
        differences.append("advertised remote branches changed")
    return "; ".join(differences)


def _repository_status(repos_root: Path, snapshot: RepositorySnapshot, repository_syncs: dict[Path, RepoSync]) -> RepositoryVersionStatus:
    ...
    current = capture_repository(
        repos_root=repos_root,
        repository_path=repository_path,
        sync=repository_syncs.get(repository_path, snapshot["sync"]),
        query_remotes=snapshot["sync"]["mode"] == "git",
    )
    ...
    detail = "Local Git state and guard configuration match" if current["sync"]["mode"] == "guard" else "Local and advertised remote state match"
    return {"snapshot": snapshot, "state": VersionRepositoryState.exact, "detail": detail}


def inspect_declared_version(repos_root: Path, declared_version: DeclaredVersion, specs_path: str | Path | None) -> list[RepositoryVersionStatus]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(_repository_status, repos_root, snapshot, repository_syncs) for snapshot in snapshots]
