...
def plan(
    ...
    try:
        impl(user_prompt=" ".join(prompt), agent=agent)
    except ValueError as error:
        raise typer.BadParameter(str(error)) from error


def execute(
    plan_json: Annotated[Path, typer.Argument(help="Plan JSON file generated by `agents plan`.")],
...
def get_app() -> typer.Typer:
    ...
    agents_app.command(name="execute", no_args_is_help=True, short_help="<E> Execute an agentops plan JSON")(execute)
    agents_app.command(name="E", no_args_is_help=True, hidden=True)(execute)
