...
def _resolve_uv_run_config(*, cwd: Path, module_file: Path) -> tuple[list[str], str | None]:
...
def use_textual_options_form(
    ...
    *,
    ...
    uv_with, uv_project_dir = _resolve_uv_run_config(cwd=Path.cwd(), module_file=Path(__file__).resolve())
    run_lambda_function(
        lambda: func(
            inputs=options,
