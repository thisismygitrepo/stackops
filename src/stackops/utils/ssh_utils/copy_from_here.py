...
def copy_from_here(
    ...
    if compress_with_zip:
        ...
        if source_obj.is_dir():
            ...
            shutil.make_archive(
                str(zip_path), "zip", source_obj.parent, source_obj.name
            )
    ...
    remote_target_full = _build_remote_path(
        self, self.remote_specs["home_dir"], target_rel2home
    )
    print(
        f"""📤 [SFTP UPLOAD] Sending file: {repr(source_obj)}  ==>  Remote Path: {remote_target_full}"""
    )
    try:
        ...
        if (
            compress_with_zip
            and source_obj.exists()
            and str(source_obj).endswith("_archive.zip")
        ):
            source_obj.unlink()
        raise
    ...
    if compress_with_zip:
        ...
        remote_zip_path = _build_remote_path(
            self, self.remote_specs["home_dir"], target_rel2home
        )
        command = lambda_to_python_script(
            lambda: unzip_archive(
                zip_file_path=remote_zip_path, overwrite_flag=overwrite_existing
            ),
            in_global=True,
            import_module=False,
        )
