...
def configure_shell_profile(
    which: Annotated[
        Literal["default", "d", "nushell", "n"], typer.Option(..., "--which", "-w", help="Which shell profile to create/configure")
    ] = "default",
):
...
def pwsh_theme():
    ...
    from machineconfig.scripts.python.helpers.helpers_devops import themes
...
def configure_wezterm_theme() -> None:
    ...
    from machineconfig.scripts.python.helpers.helpers_devops.themes import choose_wezterm_theme
...
def get_app():
    ...
    config_apps.command("windows-terminal-theme", no_args_is_help=False, help="🪟 <x> Select Windows Terminal color scheme.")(
    ...
    config_apps.command("wt-theme", no_args_is_help=False, help="Select Windows Terminal color scheme.", hidden=True)(
        configure_windows_terminal_theme
    )
