...
def upload_file(file_path: Annotated[Path, typer.Argument(..., exists=True, dir_okay=False, readable=True)]) -> None:
    """Upload a file to temp.sh (unsecure) and print the resulting URL. Use '-' as the file path to read from stdin."""
    try:
        url = _upload_file_handle(file_name=file_path.name, file_handle=file_path.read_bytes(), content_type=None)
