# StackOps Command Reference: `utils file download`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `utils file download`. Follow child links one command segment at a time.

## Current Command

- Path: `utils file download`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_utils/file_utils_app.py` -> `download`
- Help: `↓ <d> Download a file from a URL and optionally decompress it.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run utils file download --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops utils file download --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run utils file download --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
