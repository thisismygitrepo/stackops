# StackOps Command Reference: `cloud ftpx`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `cloud ftpx`. Follow child links one command segment at a time.

## Current Command

- Path: `cloud ftpx`
- Kind: `command`
- Source: `src/stackops/scripts/python/cloud.py` -> `ftpx`
- Help: `📦 File transfer utility through SSH.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run cloud ftpx --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops cloud ftpx --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run cloud ftpx --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
