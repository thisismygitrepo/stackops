# StackOps Command Reference: `agents ask`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `agents ask`. Follow child links one command segment at a time.

## Current Command

- Path: `agents ask`
- Kind: `command`
- Source: `src/stackops/scripts/python/agents.py` -> `ask`
- Help: `Ask a selected agent directly.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run agents ask --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops agents ask --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run agents ask --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
