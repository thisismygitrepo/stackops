# StackOps Command Reference: `devops data sync`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops data sync`. Follow child links one command segment at a time.

## Current Command

- Path: `devops data sync`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_data.py` -> `sync`
- Help: `🔄 <s> Back up or retrieve files and directories using rclone or share links.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops data sync --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops data sync --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops data sync --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
