# StackOps Command Reference: `terminal summary`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `terminal summary`. Follow child links one command segment at a time.

## Current Command

- Path: `terminal summary`
- Kind: `command`
- Source: `src/stackops/scripts/python/terminal_summary.py` -> `summary`
- Help: `summary.__doc__`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run terminal summary --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops terminal summary --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run terminal summary --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
