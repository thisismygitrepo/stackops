# StackOps Command Reference: `agents parallel create`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `agents parallel create`. Follow child links one command segment at a time.

## Current Command

- Path: `agents parallel create`
- Kind: `command`
- Source: `src/stackops/scripts/python/agents_parallel_commands.py` -> `agents_create`
- Help: `f"\n<c> Create agents layout file, ready to run.\n{sep}\nPROVIDER options: {', '.join(get_args(PROVIDER))}\n{sep}\nAGENT options: {', '.join(get_args(AGENTS))}\n"`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run agents parallel create --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops agents parallel create --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run agents parallel create --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
