# StackOps Command Reference: `devops data edit`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops data edit`. Follow child links one command segment at a time.

## Current Command

- Path: `devops data edit`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_data.py` -> `edit_data`
- Help: `✏️ <e> Open backup configuration file in nano, hx, or code.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops data edit --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops data edit --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops data edit --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
