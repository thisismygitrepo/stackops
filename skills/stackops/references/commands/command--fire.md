# StackOps Command Reference: `fire`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `fire`. Follow child links one command segment at a time.

## Current Command

- Path: `fire`
- Kind: `command`
- Source: `src/stackops/scripts/python/stackops_entry.py` -> `fire`
- Help: `<f> Fire and manage jobs`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run fire --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops fire --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run fire --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
