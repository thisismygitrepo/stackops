# StackOps Command Reference: `devops self docs`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops self docs`. Follow child links one command segment at a time.

## Current Command

- Path: `devops self docs`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_self_info.py` -> `docs`
- Help: `📚 <o> Serve local docs with preview URLs.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops self docs --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops self docs --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops self docs --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
