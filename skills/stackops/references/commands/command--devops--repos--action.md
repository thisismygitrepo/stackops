# StackOps Command Reference: `devops repos action`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops repos action`. Follow child links one command segment at a time.

## Current Command

- Path: `devops repos action`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_repos.py` -> `action`
- Help: `🔄 <a> Run pull/commit/push actions across repositories`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops repos action --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops repos action --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops repos action --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
