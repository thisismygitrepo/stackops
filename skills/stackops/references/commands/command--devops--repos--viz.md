# StackOps Command Reference: `devops repos viz`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops repos viz`. Follow child links one command segment at a time.

## Current Command

- Path: `devops repos viz`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_repos_viz.py` -> `gource_viz`
- Help: `🎬 <v> Visualize repository activity using Gource`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops repos viz --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops repos viz --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops repos viz --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
