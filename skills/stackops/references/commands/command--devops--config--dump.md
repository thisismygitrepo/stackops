# StackOps Command Reference: `devops config dump`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops config dump`. Follow child links one command segment at a time.

## Current Command

- Path: `devops config dump`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_config.py` -> `dump_config`
- Help: `📦 <d> Dump example configuration files and init scripts.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops config dump --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops config dump --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops config dump --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
