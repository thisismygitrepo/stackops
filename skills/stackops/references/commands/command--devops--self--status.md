# StackOps Command Reference: `devops self status`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops self status`. Follow child links one command segment at a time.

## Current Command

- Path: `devops self status`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_self.py` -> `status`
- Help: `📊 <s> STATUS of machine, shell profile, apps, symlinks, dotfiles, etc.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops self status --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops self status --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops self status --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
