# StackOps Command Reference: `devops vault search`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops vault search`. Follow child links one command segment at a time.

## Current Command

- Path: `devops vault search`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_devops/cli_vault.py` -> `search`
- Help: `<s> Retrieve credentials from Bitwarden CLI and optionally copy them to the clipboard.`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops vault search --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops vault search --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops vault search --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
