# StackOps Command Reference: `devops repos guard`

Regenerated from `src/stackops/scripts/python/graph/cli_graph.json` on 2026-06-21.

This page expands exactly one level below `devops repos guard`. Follow child links one command segment at a time.

## Current Command

- Path: `devops repos guard`
- Kind: `command`
- Source: `src/stackops/scripts/python/helpers/helpers_repos/cloud_repo_sync.py` -> `main`
- Help: `🔐 <g> Securely sync git repository to/from cloud with encryption`

## Help Commands

- `UV_CACHE_DIR=/tmp/uv-cache uv run devops repos guard --help`
- `UV_CACHE_DIR=/tmp/uv-cache uv run stackops devops repos guard --help`

## Immediate Children

- No child command references. Use `UV_CACHE_DIR=/tmp/uv-cache uv run devops repos guard --help` for options and inspect `src/stackops/scripts/python/graph/cli_graph.json` for full metadata.
